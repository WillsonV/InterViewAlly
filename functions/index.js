...
app.get("/willson", (request, response) =>
  response.status(200).send("hello Willson")
);
...
app.post("/sendOrderSucessMail/create", (req, res) => {
  ...
  var x = basket2.reduce((row, nextitem) => {
    return (
      row +
      `<tr style="border-bottom:1px solid #ddd;border-collapse:collapse;border-radius:3px;padding:8px;">
<td colspan=3 style="padding:5px;min-width:400px;"><font face="calibri" size="3"><b>` +
      nextitem.title +
      `</b></td>
<td colspan=3 style="padding:5px;min-width:120px;max-width:550px;"><font face="calibri" size="3" ><b>` +
      nextitem.quantity +
      `</b></td>
<td colspan=3 style="padding:5px;min-width:120px;max-width:550px;"><font face="calibri" size="3" ><b>` +
      nextitem.price +
